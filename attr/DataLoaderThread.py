...
class DataLoaderThread():
    ...
    def __init__(self, opt):
        ...
        for split in ['train']:
            ...
            thread = threading.Thread(target=get_batch_worker_thread, args=(split, self))
    ...
    def get_batch(self, split):
        if split == 'train':
            ...
            return get_batch_worker(split, self)


def get_batch_worker_thread(split, loader):
    while loader.flags[split]:
        data = get_batch_worker(split, loader)
        loader.queues[split].put(data)

def get_batch_worker(split, loader):
    ...
    label_batch = np.zeros([batch_size * seq_per_img, loader.seq_length], dtype='float32')

...
class MoreSupPhraseModel(nn.Module):
    def __init__(self, opt):
        super(MoreSupPhraseModel, self).__init__()
        ...
        self.logprob_pool_type = opt.logprob_pool_type
        ...
        self.phraseEmbed = Embed.PhraseEmbed(self.gram_num, self.rnn_size)
        ...
        for t in range(self.seq_length + 2):
            ...
            if t == 0:
                ...
                if not can_skip:
                    ...
                    # phrase embeddinng
                    # t - 2 + 3
                    embed_inputs[:, t - 2 + self.context_len - 1] = concat_temp.data
                    # t - 2 : t - 2 + 4
                    phrase_temp = self.phraseEmbed(Variable(embed_inputs[:, t - 2:t - 2 + self.context_len]))

                    xt = concat_temp + image_temp + phrase_temp
                    # xt = torch.cat((concat_temp.unsqueeze(1), image_temp.unsqueeze(1)), 1).mean(1).squeeze()

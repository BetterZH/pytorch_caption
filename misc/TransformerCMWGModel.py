...
class TransformerCMWGModel(nn.Module):
    def __init__(self, opt):
        super(TransformerCMWGModel, self).__init__()
        ...
        self.img_embed = nn.Linear(self.att_feat_size, self.model_size)
        self.img_relu = nn.PReLU()
        self.att_embed = nn.Linear(self.att_feat_size, self.model_size)
        self.att_relu = nn.PReLU()
        self.init_weight()

        if self.cnn_bn_drop_type == 2:
            self.img_bn = nn.BatchNorm1d(self.model_size)
            self.att_bn = BatchNorm1d(self.model_size)
    ...
    def embed_feats(self, fc_feats, att_feats):
        ...
        if self.cnn_bn_drop_type == 2:
            fc_feats = self.img_bn(fc_feats)

        fc_feats = self.img_relu(fc_feats)

        if self.cnn_bn_drop_type == 1 or self.cnn_bn_drop_type == 2:
        ...
        if self.cnn_bn_drop_type == 2:
            att_feats = self.att_bn(att_feats)

        att_feats = self.att_relu(att_feats)
    ...
    def forward(self, fc_feats, att_feats, target_seq, masks):
        ...
        output_dec, proj_wg = self.decoder(target_seq, target_pos, fc_feats, att_feats, masks)
    ...
    def sample(self, fc_feats, att_feats, opt={}):
        ...
        for i in range(self.seq_length + 1):
            ...
            output_dec, proj_wg = self.decoder(dec_partial_seq, dec_partial_pos, fc_feats, att_feats, masks_partial_seq.float())
            ...
            # output_dec:   batch_size * model_size
            logprobs = F.log_softmax(self.proj(output_dec) * proj_wg, -1)
    ...
    def sample_beam(self, fc_feats, att_feats, opt={}):
        ...
        for i in range(self.seq_length + 1):
            ...
            output_dec, proj_wg = self.decoder(dec_partial_seq, dec_partial_pos, fc_feats, output_enc, masks)
            ...
            output = F.log_softmax(self.proj(output_dec) * proj_wg, -1)

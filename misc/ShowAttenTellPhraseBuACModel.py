...
import random
class ShowAttenTellPhraseBuACModel(nn.Module):
    def __init__(self, opt):
        super(ShowAttenTellPhraseBuACModel, self).__init__()
        ...
        # VALUE
        self.value = rnn_utils.get_value(opt)
    ...
    def sample(self, fc_feats, att_feats, bu_feats, opt={}):
        ...
        state = self.init_hidden(batch_size, fc_feats)
        state_v = self.init_hidden(batch_size, fc_feats)
        ...
        for t in range(self.seq_length+2):
            ...
            state, logprobs, prob_w = self.core(xt, att_feats, bu_feats, state)
            state_v, v = self.value(xt, att_feats, bu_feats, state_v)
            if self.use_prob_weight:
                logprobs = logprobs * prob_weight
        ...
        batch_value = torch.cat([_.unsqueeze(1) for _ in values], 1)

        return batch_seq, batch_seqLogprobs, batch_value

...
class TransformerCMSModel(nn.Module):
    def __init__(self, opt):
        super(TransformerCMSModel, self).__init__()
        ...
        self.logprob_pool_type = getattr(opt, 'logprob_pool_type', 0)
        self.cnn_bn_drop_type = getattr(opt, 'cnn_bn_drop_type', 0)
        ...
        # image embedding
        # batch_size * att_size * att_feat_size
        self.img_embed = nn.Linear(self.fc_feat_size, self.model_size)
    ...
    def embed_feats(self, fc_feats, att_feats):
        ...
        if self.cnn_bn_drop_type == 1:
            if self.drop_prob_lm > 0:
                fc_feats = F.dropout(fc_feats, self.drop_prob_lm)
                att_feats = F.dropout(att_feats, self.drop_prob_lm)
    ...
    def get_logprob(self, seq_logsofts):
        # mean
        if self.logprob_pool_type == 0:
            # batch_size * len_q * output_size * n_layers
            logprob = torch.cat([_.unsqueeze(-1) for _ in seq_logsofts], -1)
            # batch_size * len_q * output_size
            logprob = logprob.mean(-1)
        # max
        elif self.logprob_pool_type == 1:
            # batch_size * len_q * output_size * n_layers
            logprob = torch.cat([_.unsqueeze(-1) for _ in seq_logsofts], -1)
            # batch_size * len_q * output_size
            logprob = logprob.max(-1)[0]
        # last
        elif self.logprob_pool_type == 2:
            # batch_size * len_q * output_size
            logprob = seq_logsofts[-1]

        # batch_size * len_q * output_size
        logprob = logprob.view_as(seq_logsofts[-1])

        return logprob

    def get_logprob_data(self, seq_logsofts):
        # mean
        if self.logprob_pool_type == 0:
            # batch_size * len_q * output_size * n_layers
            logprob = torch.cat([_.data.unsqueeze(-1) for _ in seq_logsofts], -1)
            # batch_size * len_q * output_size
            logprob = logprob.mean(-1)
        # max
        elif self.logprob_pool_type == 1:
        ...
        # last
            ...
            # batch_size * len_q * output_size
            logprob = seq_logsofts[-1].data

        # batch_size * len_q * output_size
        logprob = logprob.view_as(seq_logsofts[-1].data)
    ...
    def forward(self, fc_feats, att_feats, target_seq, masks):
        ...
        # new_masks: batch_size * len_1_q
        new_masks = torch.cat([torch.ones(batch_size, 1).cuda(), masks.data], 1)
        new_masks = Variable(new_masks)

        # target_pos: batch_size * len_1_q
        target_pos = np.array([[j + 1 for j in range(len_1_q)] for _ in range(batch_size)])
        target_pos = torch.LongTensor(target_pos).cuda()
        target_pos = Variable(target_pos, requires_grad=False)
        target_pos = target_pos * new_masks.long()
        ...
        # masks: batch_size * len_q
        # seq_logsofts: n_layers * batch_size * len_q * output_size
        seq_logsofts = self.decoder(target_seq, target_pos, fc_feats, att_feats, new_masks)
    ...
    def sample(self, fc_feats, att_feats, opt={}):
        ...
        masks = [torch.LongTensor(batch_size).fill_(1).cuda(), torch.LongTensor(batch_size).fill_(1).cuda()]
        ...
        for i in range(self.seq_length + 1):
            ...
            len_dec_1_seq = 1 + len_dec_seq

            # batch_size * len_dec_1_seq
            ...
            dec_partial_pos = np.array([[j + 1 for j in range(len_dec_1_seq)] for _ in range(batch_size)])
            ...
            seq_logsofts = self.decoder(dec_partial_seq, dec_partial_pos, fc_feats, att_feats, masks_partial_seq.float())

            seq_logsofts = self.get_logprob(seq_logsofts)

            # output_dec: batch_size * (vocab_size + 1)
            logprobs = seq_logsofts[:, -1, :].contiguous()
    ...
    def sample_beam(self, fc_feats, att_feats, opt={}):
        ...
        for i in range(self.seq_length + 1):
            ...
            masks = torch.FloatTensor(n_remaining_sents * beam_size, len_dec_1_seq).fill_(1)
            ...
            # size: 1 * len_dec_seq
            dec_partial_pos = torch.arange(1, len_dec_1_seq + 1).unsqueeze(0)
            ...
            # seq_logsofts: n_layers * (n_remaining_sents * beam_size) * len_q * (vocab_size+1)
            seq_logsofts = self.decoder(dec_partial_seq, dec_partial_pos, fc_feats, output_enc, masks)
            # seq_logsofts: (n_remaining_sents * beam_size) * len_q * (vocab_size+1)
            seq_logsofts = self.get_logprob(seq_logsofts)

            # (n_remaining_sents * beam_size) * (vocab_size+1)
            output = seq_logsofts[:,-1,:].contiguous()

            # n_remaining_sents * beam_size * (vocab_size+1)
            word_lk = output.view(n_remaining_sents, beam_size, -1)

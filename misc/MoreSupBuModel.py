...
class MoreSupBuModel(nn.Module):
    def __init__(self, opt):
        super(MoreSupBuModel, self).__init__()
        ...
        self.use_linear_type = getattr(opt, 'use_linear_type', 0)
        ...
        if self.use_linear:
            ...
            if self.use_linear_type == 0:
                self.img_embed = nn.Linear(self.fc_feat_size, self.input_encoding_size)
                self.att_embed = nn.Linear(self.att_feat_size, self.input_encoding_size)
                self.bu_embed = nn.Linear(self.bu_feat_size, self.input_encoding_size)
                # self.relu = nn.RReLU(inplace=True)
                self.img_relu = nn.ReLU()
                self.att_relu = nn.ReLU()
                self.bu_relu = nn.ReLU()
                self.init_weight()
            elif self.use_linear_type == 1:
                self.img_embed = nn.Sequential(nn.Linear(self.fc_feat_size, self.input_encoding_size),
                                               nn.ReLU(),
                                               nn.Dropout(),
                ...
                self.att_embed = nn.Sequential(nn.Linear(self.att_feat_size, self.input_encoding_size),
                                               ...
                                               nn.Dropout(),
                                               nn.Linear(self.input_encoding_size, self.input_encoding_size))
                self.bu_embed = nn.Sequential(nn.Linear(self.bu_feat_size, self.input_encoding_size),
                                               nn.ReLU(),
                                               nn.Dropout(),
                                               nn.Linear(self.input_encoding_size, self.input_encoding_size))
    ...
    def embed_feats(self, fc_feats, att_feats, bu_feats):
        ...
        if self.use_linear_type == 0:
            ...
            # batch_size * input_encoding_size <- batch_size * fc_feat_size
            fc_feats = self.img_embed(fc_feats)
            ...
            # (batch_size * att_size) * att_feat_size
            bu_feats = bu_feats.view(-1, self.att_feat_size)
            # (batch_size * att_size) * input_encoding_size
            bu_feats = self.bu_embed(bu_feats)
            # batch_size * att_size * input_encoding_size
            bu_feats = bu_feats.view(-1, self.att_size, self.input_encoding_size)
    ...
    def forward(self, fc_feats, att_feats, bu_feats, labels):
        ...
        for t in range(self.seq_length + 2):
            ...
            if t == 0:
                ...
                if random.randint(0,99) >= self.sample_rate:
                    ...
                    logprob = self.get_logprob(logprobs)
                    if sample_max == 1:
                        ...
                        if temperature == 1.0:
                            prob_prev = torch.exp(logprob.data)
                        else:
            ...
            if not can_skip:
                state, logprobs = self.core(xt, att_feats, bu_feats, state)
                if t > 0:
        ...
        for i in range(self.num_layers):
            batch_output = torch.cat([_[i].unsqueeze(1) for _ in outputs], 1).contiguous()
    ...
    # fc_feats   batch_size * feat_size
    # att_feats  batch_size * att_size * feat_size
    def sample_forward(self, fc_feats, att_feats, bu_feats, labels, opt={}):
        ...
        for t in range(self.seq_length + 2):
            if t == 0:
                ...
                if sample_max == 1:
                    # sampleLogprobs -> current sample logprobs
                    # it -> current index
                    sampleLogprobs, it_ = torch.max(logprob, 1)
                    it_ = it_.data.view(-1).long()
                else:
                    if temperature == 1.0:
                        prob_prev = torch.exp(logprob.data)
                    else:
                        prob_prev = torch.exp(torch.div(logprob.data, temperature))
                    it_ = torch.multinomial(prob_prev, 1)
                    sampleLogprobs = logprob.gather(1, Variable(it_))
                    it_ = it_.view(-1).long()

                if random.randint(0, 99) >= self.sample_rate:
                    it = labels[:, t - 2].clone()
                else:
                    it = Variable(it_.view(-1).long())
        ...
        for t in range(self.seq_length+2):
            if t == 0:
                ...
                if sample_max == 1:
                    # sampleLogprobs -> current sample logprobs
                    # it -> current index
                    sampleLogprobs, it = torch.max(logprob, 1)
                    ...
                    if temperature == 1.0:
                        ...
                        prob_prev = torch.exp(torch.div(logprob.data, temperature))
                    it = torch.multinomial(prob_prev, 1)
                    sampleLogprobs = logprob.gather(1, Variable(it))
                    it = it.view(-1).long()
    ...
    def sample_beam(self, fc_feats, att_feats, bu_feats, opt={}):
        ...
        for k in range(batch_size):
            ...
            for t in range(self.seq_length+2):
                ...
                state, logprobs = self.core(xt, beam_att_feats, beam_bu_feats, state)

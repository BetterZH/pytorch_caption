...
class MoreSupModel(nn.Module):
    def __init__(self, opt):
        super(MoreSupModel, self).__init__()
        ...
        if self.use_linear:
            ...
            if self.use_linear_type == 0:
                ...
                self.att_embed = nn.Sequential(nn.Linear(self.fc_feat_size, self.input_encoding_size),
                                               nn.ReLU(),
                                               nn.Dropout(),
                                               nn.Linear(self.input_encoding_size, self.input_encoding_size))
    ...
    # fc_feats   batch_size * feat_size
    # att_feats  batch_size * att_size * feat_size
    def sample_forward(self, fc_feats, att_feats, labels, opt={}):

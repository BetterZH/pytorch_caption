...
class ShowAttenTellPhraseModel(nn.Module):
    def __init__(self, opt):
        ...
        if self.use_linear_embedding == 1:
            self.embed = nn.Linear(self.vocab_size + 1, self.input_encoding_size)
    ...
    def forward(self, fc_feats, att_feats, labels):
        ...
        for t in range(self.seq_length + 2):
            ...
            if not can_skip:
                output = self.core(xt, att_feats, state)
                state = output[0]
                logprobs = output[1]
                if self.use_prob_weight:
                    logprobs = F.log_softmax(logprobs * prob_weight)
                    prob_w = prob_weight
                else:
                    prob_w = output[2]
                if t > 0:
    ...
    def sample_beam(self, fc_feats, att_feats, opt={}):
        ...
        if self.use_linear:
            fc_feats_new, att_feats_new = self.embed_feats(fc_feats, att_feats)
        ...
        for k in range(batch_size):
            ...
            for t in range(self.seq_length+2):
                ...
                output = self.core(xt, beam_att_feats, state)
                state = output[0]
                logprobs = output[1]
                if self.use_prob_weight:
                    logprobs = F.log_softmax(logprobs * prob_weight)
